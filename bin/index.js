...
let startDev = async () => {
    const { stdout, stderr } = await exec('yarn start-dev');
    log(stdout);
    error(stderr);
};
...
program
    ...
    .option('-s, --setup_mode [mode]', "Which setup mode to use. 'blank' for bare bones project")
    .action((project_name, project_path, options) => {
        let mode = options.setup_mode || 'starter';
        let path = project_path || `./${project_name}`;

        (welcome = () => {
            console.log(colors.green.bgWhite.bold.underline(`${CLI_NAME} -v ${CLI_VERSION}`));
        ...
        let success = () => {
            ...
            warn(`cd into ${path} and type '${CLI_NAME} run' to start the project locally\n`);
        };
    });
...
program
    ...
    .option('-m, --mode [mode]', "Blank for both server/client, s for just server, c for just client")
    .action((options) => {
